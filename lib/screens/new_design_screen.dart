...
class _NewDesignScreenState extends State<NewDesignScreen> {
  ...
  void _applyPreset(int index) {
    ...
    final w = double.tryParse(preset['w'] as String) ?? 10;
    final h = double.tryParse(preset['h'] as String) ?? 15;
    setState(() {
  ...
  Widget build(BuildContext context) {
    // Manejo del botón atrás del celular con NavigatorObserver
    return WillPopScope(
      onWillPop: () async {
        _goHome();
        return false;
      ...
      child: Column(
        children: [
              ...
              return Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: isActive ? AppTheme.accentRed : _cardColor,
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(
                        color: isActive ? AppTheme.accentRed : _borderColor,
                        width: 1,
                      ),
                    ),
                    child: Center(
          ...
          Row(
            children: [
              Expanded(
                child: _buildDimensionField('Ancho', _widthController),
              ),
              GestureDetector(
              ...
              Expanded(
                child: _buildDimensionField('Alto', _heightController),
              ),
            ],
          ),
        ],
        ...
        children: CanvasDpi.values.map((dpi) {
          ...
          switch (dpi) {
            case CanvasDpi.dpi72:
              label = '72'; sub = 'Pantalla'; break;
            case CanvasDpi.dpi150:
              label = '150'; sub = 'Estándar'; break;
            case CanvasDpi.dpi300:
              label = '300'; sub = 'Impresión'; break;
          }
          return Expanded(
            child: GestureDetector(
  ...
  Widget _buildOrientationOption(
      CanvasOrientation orientation, IconData icon, String label) {
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              Icon(icon,
                  color: isActive ? AppTheme.accentRed : _textSecondary,
                  size: 24),
      ...
      child: Row(
        children: [
          _buildBgOption(
            ...
            Stack(
              fit: StackFit.expand,
              children: [CustomPaint(painter: _CheckerPainter())],
            ),
          ),
          const SizedBox(width: 10),
          _buildBgOption(
  ...
  Widget _buildPreview() {
    ...
    switch (_background) {
      case CanvasBackground.blanco:
        bgColor = Colors.white; break;
      case CanvasBackground.negro:
        bgColor = Colors.black; break;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double y = 0; y < size.height; y += tileSize) {
      for (double x = 0; x < size.width; x += tileSize) {
        final isEven =
            ((x / tileSize).round() + (y / tileSize).round()) % 2 == 0;
        canvas.drawRect(
